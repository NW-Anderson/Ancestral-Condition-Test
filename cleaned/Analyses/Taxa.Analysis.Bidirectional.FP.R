...
p.val.array <- array(dim = c(n.trees, length(n.taxa)))

# this time we vary the size of the tree
for(s in 1:length(n.taxa)){
  # for each tree size we repeat the same analysis for the same number of trees
  p.val.vec <- foreach(t = 1:n.trees, .options.multicore=opts, .combine = 'c', 
