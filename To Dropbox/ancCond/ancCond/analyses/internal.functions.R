...
CreateNull <- function(tree,                     # a tree type phylo
  ...
  for(n in 1:iter){
    ...
    good.sim <- F
    while(good.sim == F){
      sim.anc.state.dt <- sim.history(tree=tree, Q=current.Q, nsim=1, message = F)
...
PrintStatements <- function(results){
